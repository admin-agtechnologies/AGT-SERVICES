# AGT Notification Service v1.0 - Setup (Windows PowerShell)

Write-Host "AGT Notification Service v1.0 - Setup" -ForegroundColor Cyan
...
if (-not (Test-Path "keys\auth_public.pem")) {
    ...
    if (Test-Path "..\agt-auth\keys\public.pem") {
        ...
        Write-Host "[OK] Cle publique Auth copiee" -ForegroundColor Green
    }
    else {
        Write-Host "[WARN] Cle publique Auth introuvable dans ..\agt-auth\keys\public.pem" -ForegroundColor Yellow
        Write-Host "       Copiez-la : copy <chemin>\public.pem keys\auth_public.pem" -ForegroundColor Yellow
    }
}
else {
...
# 3. Build et Start (API + Worker + Beat + RabbitMQ)
Write-Host ""
...
# 4. Attente (RabbitMQ est plus lent a demarrer)
Write-Host "Attente demarrage services (RabbitMQ + Celery)..." -ForegroundColor Yellow
...
try {
    $health = Invoke-RestMethod -Uri "http://localhost:7002/api/v1/health" -Method GET
...
Write-Host ""
Write-Host "[OK] Notification Service pret sur http://localhost:7002" -ForegroundColor Green
Write-Host ""
Write-Host "Documentation API :" -ForegroundColor Cyan
Write-Host "  Swagger UI : http://localhost:7002/api/v1/docs/" -ForegroundColor Gray
Write-Host "  RabbitMQ   : http://localhost:15672 (guest/guest)" -ForegroundColor Gray
Write-Host ""
Write-Host "Tests : docker compose exec notification python -m pytest -v" -ForegroundColor Gray
