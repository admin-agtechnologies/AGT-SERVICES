# AGT Wallet Service v1.0 - Setup (Windows PowerShell)
Write-Host "AGT Wallet Service v1.0 - Setup" -ForegroundColor Cyan
$dockerOk = $false
...
Write-Host ""
try { $h = Invoke-RestMethod -Uri "http://localhost:7006/api/v1/wallet/health"; $h | ConvertTo-Json } catch { Write-Host "En cours..." -ForegroundColor Yellow }
Write-Host ""
Write-Host "[OK] Wallet Service pret sur http://localhost:7006" -ForegroundColor Green
Write-Host "  Swagger : http://localhost:7006/api/v1/docs/" -ForegroundColor Gray
Write-Host "  ReDoc   : http://localhost:7006/api/v1/redoc/" -ForegroundColor Gray
Write-Host "  Tests   : docker compose exec wallet python -m pytest -v" -ForegroundColor Gray
